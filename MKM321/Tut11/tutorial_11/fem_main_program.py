...
def launch_fem(case, MagFac=1.0, IntegrationType = 'FI'):
    ...
    #Element types 2, 4 and 6 are plane stress
    #Element types 1, 3 and 5 are plain strain
    elas = DD['EMODULUS'][0][0]; pois = DD['POISSON'][0][0]

    if np.mod(DD['ELTYPE'][0][0], 2) == 1:
        # plane strain
        matD = elas/((1+pois)*(1 - 2*pois))*np.asarray([[1-pois,pois,0],[pois,1-pois,0],[0,0,1/2-pois]])
        plane = 1
    else:
        # plane stress
        matD = elas/(1-pois**2)*np.asarray([[1,pois,0],[pois,1,0],[0,0,(1-pois)/2]])
        plane = 0
...
#        PlotIso(Xvec,Yvec,IsoSm,trisnew,'Isochromatic',Scale)
    ...
    return U, Fp, VonMises, S11, S22, S12
