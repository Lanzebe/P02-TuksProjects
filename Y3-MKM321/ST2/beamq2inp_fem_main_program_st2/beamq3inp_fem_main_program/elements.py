...
def Quad4R_Stiffness(X, Cmat, t):
    import numpy as np
    Tangent = np.matrix(np.zeros([8, 8]))  # Initialize tangent with zeros

    for jGauss in range(1):    # 1 by 1 Gauss integration loops
        eta = 0  # Natural coordinate eta
        for iGauss in range(1):
            xi = 0   # Natural coordinate xi
            [B, detJ] = B_Quad4(xi, eta, X)     # B for Eq.(4.83)
            Tangent = Tangent + 2 * 2 * B.T * Cmat * B * detJ * t
    return Tangent